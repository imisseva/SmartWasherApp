...
import { UserController, AdminUserVM, CreateUserDto, UpdateUserDto } from "../../controllers/UserController";
...
export default function UsersScreen() {
  ...
  const renderItem = ({ item }: { item: AdminUserVM }) => (
    <View style={styles.row}>
      <View style={{ flex: 1 }}>
        ...
        <Text style={[styles.role, item.role === "admin" ? styles.badgeAdmin : styles.badgeUser]}>
          {item.role}
        </Text>
      </View>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "hsl(218, 50%, 91%)" }}>
      {/* Header custom (thấp – không dính tai thỏ) */}
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={{ padding: 16 }}>Chưa có người dùng.</Text>}
      />
...
const styles = StyleSheet.create({
  ...
  role: { marginTop: 8, alignSelf: "flex-start", paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 },
